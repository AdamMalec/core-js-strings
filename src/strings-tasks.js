...
function removeFirstOccurrences(str, value) {
  ...
  return `${str.slice(0, indexOf)}${str.slice(indexOf + valueLength)}`;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndexOf === -1) return str;
  const valueLength = value.length;
  return `${str.slice(0, lastIndexOf)}${str.slice(lastIndexOf + valueLength)}`;
}
...
function findLongestWord(sentence) {
  ...
  wordsArr.forEach((item) => {
    if (item.length > largestLength) largestLength = item.length;
  });

  for (let i = 0; i < wordsArr.length; i += 1) {
    if (wordsArr[i].length === largestLength) return wordsArr[i];
  }
...
function encodeToRot13(str) {
  ...
  strArr.forEach((item) => {
    ...
    const index = alphabet.indexOf(item.toLowerCase());

    if (index <= 12 && index !== -1) {
      if (isUpper) resultArr.push(alphabet[index + 13].toUpperCase());
      else resultArr.push(alphabet[index + 13]);
